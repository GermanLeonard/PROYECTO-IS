...
const Cancha = () => {
  ...
  const { canchas, moneda, backendUrl, token, getCanchasData } =
  ...
  const horariosDisponibles = async () => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      while (fechaActual < horaFin) {
        let formatoTiempo = fechaActual.toLocaleTimeString([], {
          hour: "2-digit",
          minute: "2-digit",
        });
  ...
  const reservarCancha = async () => {
    ...
    try {
        let fecha = reservaFecha[fechaIndex][0].fecha;
        ...
        console.log("Datos a enviar:", { canchaId, espacioFecha, reservaHora, fechaISO });
